...
if(SHIBOKEN_BASEDIR)
    execute_process(
        COMMAND ${Python3_EXECUTABLE} -c "if True:
            import os
            import shiboken6.Shiboken as Shiboken
            print(os.path.basename(Shiboken.__file__).split('.', 1)[1])
        "
        OUTPUT_VARIABLE SHIBOKEN_SUFFIX
        OUTPUT_STRIP_TRAILING_WHITESPACE
    )
    ...
    execute_process(
        COMMAND ${Python3_EXECUTABLE} -c "if True:
            import os
            import shiboken6.Shiboken as Shiboken
            print(';'.join(map(str, Shiboken.__version_info__)))
        "
        OUTPUT_VARIABLE SHIBOKEN_SO_VERSION
        OUTPUT_STRIP_TRAILING_WHITESPACE
    )
    list(GET SHIBOKEN_SO_VERSION 0 SHIBOKEN_SO_MACRO_VERSION)
    list(GET SHIBOKEN_SO_VERSION 1 SHIBOKEN_SO_MICRO_VERSION)
    list(GET SHIBOKEN_SO_VERSION 2 SHIBOKEN_SO_MINOR_VERSION)
    string(REPLACE ";" "." SHIBOKEN_SO_VERSION "${SHIBOKEN_SO_VERSION}")

    if(NOT APPLE)
        set(SHIBOKEN_SUFFIX "${SHIBOKEN_SUFFIX}.${SHIBOKEN_SO_MACRO_VERSION}.${SHIBOKEN_SO_MICRO_VERSION}")
    else()
        string(REPLACE ".so" "" SHIBOKEN_SUFFIX ${SHIBOKEN_SUFFIX})
        set(SHIBOKEN_SUFFIX "${SHIBOKEN_SUFFIX}.${SHIBOKEN_SO_MACRO_VERSION}.${SHIBOKEN_SO_MICRO_VERSION}.dylib")
    endif()
    message(STATUS "Shiboken suffix:            ${SHIBOKEN_SUFFIX}")
